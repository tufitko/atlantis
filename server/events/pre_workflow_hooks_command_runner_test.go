...
func TestRunPreHooks_Clone(t *testing.T) {
	...
	t.Run("success hooks in cfg", func(t *testing.T) {
		...
		When(preWhWorkingDir.CloneFull(Any[logging.SimpleLogging](), Eq(testdata.GithubRepo), Eq(newPull),
	...
	t.Run("success hooks not in cfg", func(t *testing.T) {
		...
		preWhWorkingDir.VerifyWasCalled(Never()).CloneFull(Any[logging.SimpleLogging](), Eq(testdata.GithubRepo), Eq(newPull),
