...
func (w *DefaultPreWorkflowHooksCommandRunner) RunPreHooks(ctx *command.Context, cmd *CommentCommand) error {
	...
	if cmd.Name == command.Plan {
		repoDir, err = w.WorkingDir.CloneForce(ctx.Log, ctx.HeadRepo, ctx.Pull, DefaultWorkspace)
		if err != nil {
			return err
		}
	}
	...
	if cmd.Name == command.Plan {
		repoDir, err = w.WorkingDir.CloneCheckUpstreams(ctx.Log, ctx.HeadRepo, ctx.Pull, DefaultWorkspace)
		if err != nil {
			return err
		}
	}
